...
class SudoWifi:
    ...
    def scan_with_bssid(self,interface, bssid):
        ...
        command = [
            "sudo", "airodump-ng", interface,
            "--bssid", bssid,
            "--write-interval", "1", "--output-format", "csv", "--write", "output_file",">","/dev/null","2>&1","&"
        ]

        # Start the airodump-ng process
        #process = subprocess.Popen(command, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        process=os.popen(f"sudo airodump-ng {interface} --bssid {bssid} --write-interval 1 --output-format csv --write output_file > /dev/null 2>&1 &")
